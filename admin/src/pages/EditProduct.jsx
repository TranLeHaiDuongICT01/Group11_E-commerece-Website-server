...
const EditProduct = () => {
    ...
    const [images, setImages] = useState([...data?.images?.map(image => image.url)])
            ...
            <Paper elevation={6} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', marginTop: '20px', padding: '20px' }}>
                ...
                <Box sx={{ width: '100%' }}>
                    <form onSubmit={handleSubmit} component={FormControl} style={{
                        ...
                        <CheckGroup list={categoryList} name='category' data={data} setData={setData} setClear={setClear} />
                        <CheckGroup list={sizeList} name='size' data={data} setData={setData} setClear={setClear} />
                        <CheckGroup list={colorList} name='color' data={data} setData={setData} setClear={setClear} />
                        <div style={{
                            position: 'relative',
                            display: 'flex',
                            flexWrap: 'wrap',
                        ...
                        }}>
                            {
                                images?.map((image, i) => (
                                    <div key={i} style={{
                                        ...
                                        <CardMedia component='img' sx={{ width: '100%', objectFit: 'cover', maxHeight: '700px', height: '100%' }} id='imageSeen' image={image} alt='' />
                                    </div>
                                ))
                            }
                        </div>
