...
const Login = () => {
    ...
    const googleSuccess = async (res) => {
        ...
        try {
            ...
            dispatch(loginFailure(error?.message || "Google sign in unsuccessfully. Try again"))
        }
    }
    ...
    return (
        <Container>
            <Wrapper>
                <Title>LOG IN</Title>
                <Form>
                    <Input placeholder="Email" name="email" value={formData.name} onChange={inputHandler} required />
                    <Input placeholder="Password" name="password" value={formData.password} onChange={inputHandler} required />
                    <GoogleLogin fullWidth
                        clientId='694830871542-d8ggebospri9avtnpi995163epoe0d7d.apps.googleusercontent.com'
                        render={(renderProps) => (
                            <Button style={{ backgroundColor: 'teal', marginTop: '20px' }}
                    ...
                    />
                    <Button fullWidth variant='contained' style={{ backgroundColor: 'teal', margin: '20px 0' }} onClick={handleClick}>LOG IN</Button>
                    {
