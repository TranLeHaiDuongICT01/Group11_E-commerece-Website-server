...
const OrderHistory = () => {
    ...
    return (
        !error && !isFetching && orders && orders.length > 0 ? (
            <Container className='container-history' sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <Typography variant='h5'>You have {orders?.length} {orders?.length === 1 ? 'order' : 'orders'}</Typography>
                        ...
                        <tbody>
                            {
                                orders?.map(item => (
                                    <tr key={item?._id}>
                                        ...
                                        <td><Link to={`/order/${item?._id}`}>View</Link></td>
                                    </tr>
                                ))
                            }
                        </tbody>
