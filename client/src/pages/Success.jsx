...
const Success = () => {
  ...
  return (
    ...
    >
              ...
              <tbody>
                {
                  cart?.products?.map((item, i) => (
                    <tr key={i}>
                      <td><img style={{ width: '80px', height: '80px', objectFit: 'cover' }} src={item?.images[0]?.url} alt={item?.title} /></td>
                      <td>{item?.title}</td>
                      <td>{item?.quantity}</td>
                      <td>{item?.price}</td>
                    </tr>
      ...
      <button style={{ padding: 10, marginTop: 20 }} onClick={() => navi('/')}>Go to Homepage</button>
    </div>
  )
}
